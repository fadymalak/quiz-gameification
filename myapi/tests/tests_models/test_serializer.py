...
@pytest.mark.curr
@pytest.mark.parametrize('r',["MCQ","GQ","YNQ"])
def test_serializer_question_update_partial(r,CREATE_QUESTION):
...
@pytest.mark.curr
@pytest.mark.parametrize('r',["MCQ","GQ","YNQ"])
def test_serializer_question_update_full(r,CREATE_QUESTION):
    '''PUT/UPDATE  NEED UPDATE & partial UPDATE'''
    for i in BaseItemSerializer.__subclasses__():
        if r.lower() in i.__name__.lower():
            serial = i(CREATE_QUESTION)
    ...
    print(question.point)
    data["title"] = "Questionssss"
    question_data = {"id":question.id, "item":data,"quiz":quiz.id,"point":20}
    question_serial = QuestionSerializer(question,data=question_data)
    question_serial.is_valid()
    question_serial.save()
    print(question_serial.data)
    assert 1==1